...
const Notifications: React.FC<NotificationsProps> = ({ notifications, currentUser, onSetRead }) => {
    ...
    return (
        <div className="relative z-[60]" ref={dropdownRef}>
            <button onClick={handleToggle} className="relative p-2 rounded-full text-dimmed hover:bg-dark/50 hover:text-accent transition-colors">
            ...
            {isOpen && (
                <div className="absolute right-0 mt-2 w-80 bg-medium rounded-lg shadow-xl overflow-hidden text-right border border-dark">
                    <div className="p-3 font-bold text-primary border-b border-dark">
                        התראות
                    </div>
                    <div className="max-h-96 overflow-y-auto">
                        {userNotifications.length > 0 ? (
                            userNotifications.map(n => (
                                <div key={n.id} className={`p-3 border-b border-dark/50 ${!n.read ? 'bg-accent/20' : ''}`}>
                                    <p className="text-sm text-primary">{n.text}</p>
                                    <p className="text-xs text-dimmed mt-1">{timeSince(n.timestamp)}</p>
                                </div>
                            ))
                        ) : (
                            <div className="p-4 text-center text-sm text-dimmed">
                                אין לך התראות חדשות.
                            </div>
                        )}
                    </div>
